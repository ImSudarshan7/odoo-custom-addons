...
(function($) { // Hide scope, no $ conflict
	...
	/** Implementation of the Nepali civil calendar.
		...
		See also <a href="http://en.wikipedia.org/wiki/nepali_date">http://en.wikipedia.org/wiki/nepali_date</a>
	...
	$.extend(NepaliCalendar.prototype, {
		...
		daysInYear: function(year) {
			...
			year = date.year();
			if (typeof this.nepali_date_DATA[year] === 'undefined') {
				return this.daysPerYear;
			}
			var daysPerYear = 0;
			for (var month = this.minMonth; month <= 12; month++) {
				daysPerYear += this.nepali_date_DATA[year][month];
			}
			return daysPerYear;
		},
		...
		daysInMonth: function(year, month) {
			...
			return (typeof this.nepali_date_DATA[year] === 'undefined' ?
				this.daysPerMonth[month - 1] : this.nepali_date_DATA[year][month]);
		},
		...
		toJD: function(nepaliYear, nepaliMonth, nepaliDay) {
			...
			var gregorianYear = nepaliYear - (nepaliMonthToCheck > 9 || (nepaliMonthToCheck === 9 &&
				nepaliDay >= this.nepali_date_DATA[nepaliYearToCheck][0]) ? 56 : 57);
			...
			while (nepaliMonthToCheck !== 9) {
				...
				gregorianDayOfYear += this.nepali_date_DATA[nepaliYearToCheck][nepaliMonthToCheck];
				nepaliMonthToCheck--;
			}
			...
			if (nepaliMonth === 9) {
				...
				gregorianDayOfYear += nepaliDay - this.nepali_date_DATA[nepaliYearToCheck][0];
				...
				gregorianDayOfYear += this.nepali_date_DATA[nepaliYearToCheck][9] -
		...
		fromJD: function(jd) {
			...
			var dayOfFirstJanInPaush = this.nepali_date_DATA[nepaliYear][0];
			...
			var daysSinceJanFirstToEndOfNepaliMonth =
				this.nepali_date_DATA[nepaliYear][nepaliMonth] - dayOfFirstJanInPaush + 1;
			...
			// The next nepali month (Magh) has 29 days
			...
			// we calculated there are 43 days from 1st. January (17 Paush) till end of Magh (29 days)
		...
		/** Creates missing data in the nepali_date_DATA table.
		...
		_createMissingCalendarData: function(nepaliYear) {
			...
			for (var nepaliYearToCreate = (nepaliYear - 1); nepaliYearToCreate < (nepaliYear + 2); nepaliYearToCreate++) {
				if (typeof this.nepali_date_DATA[nepaliYearToCreate] === 'undefined') {
					this.nepali_date_DATA[nepaliYearToCreate] = calendarData;
				}
			}
		},
