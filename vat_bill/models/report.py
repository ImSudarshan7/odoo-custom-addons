...
class PDFReport(models.Model):
    ...
    def _post_pdf(self, save_in_attachment, pdf_content=None, res_ids=None):
        ...
        if result:
            ...
            watermark = last_page = None
            if report.name == 'VAT Invoice':
                watermark = self.env.company.pdf_watermark or None
                last_page = report.pdf_last_page or report.env.company.pdf_last_page or None
            if report.name == 'Helpdesk Ticket':
                watermark = report.pdf_watermark or self.env.company.pdf_watermark or None
                last_page = report.pdf_last_page or report.env.company.pdf_last_page or None
                ...
                try:
                    ...
                    if pdf_watermark.isEncrypted:
                        try:
                            ...
                            pdf_watermark = None
                            msg = _(
                                'The uploaded watermark PDF document has security restrictions. '
                                'Can not read or decrypt it!: '
                            )
                            msg += str(e)
                            logger.warning(msg)
                            raise UserError(msg)
