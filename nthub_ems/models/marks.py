...
class Marks(models.Model):
    ...
    #        If the education record ID is provided in the values list, it retrieves related information such as student ID and office year.
    #        """
    #     vals_list[0]['is_readonly'] = True
    #     if vals_list[0]['education_record_id']:
    #         vals_list[0]['student_id'] = self.env['education.record'].sudo().browse(
    #             vals_list[0]['education_record_id']).student_id.id
    #         vals_list[0]['office_year'] = self.env['education.record'].sudo().browse(
    #             vals_list[0]['education_record_id']).yro
    #     else:
    #         edu_record = self.env['education.record'].sudo().search(
    #             [('student_id', '=', vals_list[0]['student_id']), ('yro', '=', vals_list[0]['office_year'])])
    #         if edu_record:
    #             vals_list[0]['education_record_id'] = edu_record.id
    #         else:
    #             raise ValidationError(_('This student has no Education Record in this year.'))
    #     return super(Marks, self).create(vals_list)
    ...
    def make_generate_records(self):
        return {
