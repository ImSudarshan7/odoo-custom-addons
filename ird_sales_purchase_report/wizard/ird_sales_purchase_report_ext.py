...
class IrdSalesPurchaseReport(models.TransientModel):
    ...
    end_date = fields.Date(string="End Date", required=True, default=datetime.now().replace(
    ...
    def action_ird_sales_purchase_report_ext(self):
        ...
        if self.invoice_type == 'out_invoice':
            ...
            format0 = xlwt.easyxf(
                'font:height 500,bold True;pattern: pattern solid, fore_colour gray25;align: horiz center')
            ...
            format8 = xlwt.easyxf(
                'font:bold True;borders:top thick;pattern: pattern solid, fore_colour gray25;align: horiz left')
        ...
        if invoice:
            ...
            nepali_start_date_str = self.convert_to_nepali_date(self.start_date.strftime('%Y-%m-%d'))
            nepali_end_date_str = self.convert_to_nepali_date(self.end_date.strftime('%Y-%m-%d'))

            today_dt = datetime.today()
            nepali_date_part_str = self.convert_to_nepali_date(today_dt.strftime('%Y-%m-%d'))
            nepali_timestamp_str = f"{nepali_date_part_str} {today_dt.strftime('%H:%M:%S')}"
            row = 11
            if self.invoice_type == 'out_invoice':
                sheet.write(4, 0, "Seller Name:", format1)
                sheet.write(4, 1, str(self.env.company.name), format2)
                # _logger.info("====%s==========rec=====%s===",self,self.env.company.name)
                ...
                sheet.write(7, 1, nepali_start_date_str, format2)
                sheet.write(7, 2, 'TO', format2)
                sheet.write(7, 3, nepali_end_date_str, format2)
                sheet.write(8, 0, 'Report generated at:', format1)
                sheet.write(8, 1, nepali_timestamp_str, format2)
                ...
                if record:
                    ...
                    for rec in record:
                        ...
                        for lines in rec.invoice_line_ids:
                            nepali_date_invoice_str = self.convert_to_nepali_date(
                                rec.invoice_date.strftime('%Y-%m-%d'))
                            sheet.write(row, 0, nepali_date_invoice_str, format3)
                ...
                if record:
                    ...
                    for rec in record:
                        for lines in rec.invoice_line_ids:
                            ...
                            #     sheet.write(row, 2, str(rec.partner_id.parent_id.name), format3)
                            #     if rec.partner_id.parent_id.vat:
                            #         sheet.write(row, 3, str(rec.partner_id.vat), format3)
                            #     else:
                            #         sheet.write(row, 3, "", format3)
                ...
                if record:
                    ...
                    for rec in record:
                        for lines in rec.invoice_line_ids:
                            ...
                            sheet.write(row1, 0, nepali_date_invoice_str, format3)
            ...
            path = ("/home/odoo/Reports/IRD_Sales_Purchase_Book_Ext.xls")
