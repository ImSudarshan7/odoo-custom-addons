...
class DashboardBlock(models.Model):
    ...
    def get_dashboard_vals(self, action_id):
        """Dashboard block values with user-based filtering"""
        block_id = []
        user = self.env.user
        ...
        for rec in dashboard_block:
            ...
            if not user.has_group('base.group_system'):  # Non-admin user
            ...
            if rec.model_name:
                if rec.type == 'graph':
                    ...
                    x_axis = [record.get(rec.group_by.name) for record in records]
                    ...
                    records = self._cr.dictfetchall()
                    magnitude = 0
                    total = records[0].get('value') if records else 0
                    while abs(total) >= 1000:
                        magnitude += 1
                        total /= 1000.0
                    val = '%.2f%s' % (total, ['', 'K', 'M', 'G', 'T', 'P'][magnitude])
                    records[0] = {'value': val}
                    vals.update(records[0] if records else {})
